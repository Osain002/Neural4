    ...
    def build_conv_layers(self, input_size):
        ...
        for layer in self.convNet_layers:
            ...
            if layer["layer_type"] == "conv":
                ...
                out_size = tuple(
                    [int(((input_size[i] + 2*padding - filter_shape[i])/layer["stride"]) + 1)for i in range(2)])
                layer["bias"] = [np.random.randn(
                    out_size[0], out_size[1]) for i in range(len(layer["filters"]))]
                ...
                out_size = tuple(
                    [int(((input_size[i] + 2*padding - kernal_size)/stride) + 1)for i in range(2)])

            layer["out_size"] = out_size
            input_size = out_size

        flat_net_outsize = np.prod(
            self.convNet_layers[-1]["out_size"])*total_feature_maps
