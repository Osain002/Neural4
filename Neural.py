...
import Layer_operations.conv.backprop_conv as convbp
...
class Net_frame(Layers, dense_ff, dense_bp, conv_ff, convbp):
    ...
    def __init__(self) -> None:
        ...
        convbp.__init__(self, self.convNet_layers, self.dense_layers)
        ...
        while epoch < epochs:
            for i, input in enumerate(x_train):
                ...
                self.calculate_error(
                    self.dense_layers[-1]["layer_out"], [target])
                self.run_backprop(target, learning_rate)
